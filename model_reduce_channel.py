...
def resnet_model_keras(num_classes):
    ...
    x = _conv_bn(x, 32, (5, 5), (2, 2), "conv1", "bn_conv1", padding="valid")
    x = layers.Activation("relu")(x)
    ...
    # fliter1 = 64
    fliter1 = 32
    x = _conv_block(x, (3, 3), [fliter1, fliter1, fliter1 * 4], stage=2, block='a', strides=(1, 1))
    x = _identity_block(x, (3, 3), [fliter1, fliter1, fliter1 * 4], stage=2, block='b')
    x = _identity_block(x, (3, 3), [fliter1, fliter1, fliter1 * 4], stage=2, block='c')

    # block_layer 2
    # filter2 = 128
    filter2 = 64
    x = _conv_block(x, (3, 3), [filter2, filter2, filter2 * 4], stage=3, block='a')
    x = _identity_block(x, (3, 3), [filter2, filter2, filter2 * 4], stage=3, block='b')
    x = _identity_block(x, (3, 3), [filter2, filter2, filter2 * 4], stage=3, block='c')
    x = _identity_block(x, (3, 3), [filter2, filter2, filter2 * 4], stage=3, block='d')

    # block_layer 3
    # filter2 = 256
    filter3 = 128
    x = _conv_block(x, 3, [filter3, filter3, filter3 * 4], stage=4, block='a')

    x = _identity_block(x, 3, [filter3, filter3, filter3 * 4], stage=4, block='b')

    x = _identity_block(x, 3, [filter3, filter3, filter3 * 4], stage=4, block='c')

    x = _identity_block(x, 3, [filter3, filter3, filter3 * 4], stage=4, block='d')

    x = _identity_block(x, 3, [filter3, filter3, filter3 * 4], stage=4, block='e')

    x = _identity_block(x, 3, [filter3, filter3, filter3 * 4], stage=4, block='f')

    # block_layer 4
    # filter2 = 512
    filter4 = 256
    x = _conv_block(x, 3, [filter4, filter4, filter4 * 4], stage=5, block='a')

    x = _identity_block(x, 3, [filter4, filter4, filter4 * 4], stage=5, block='b')

    x = _identity_block(x, 3, [filter4, filter4, filter4 * 4], stage=5, block='c')
